...
public final class SessionNetwork {
    ...
    public static synchronized void register() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(NetworkConstants.EDIT_DUEL_TYPE_ID, (payload, context) -> {
            if (!SessionPermissions.checkCanManageSessions(context.player(), "edit duel types")) return;
            String id = payload.id();
            if (dev.frost.miniverse.minigame.impl.duels.DuelTypeRegistry.get(id).isEmpty()) return;
            String name = payload.name() == null ? "" : payload.name().trim();
            if (name.isBlank()) {
                context.player().sendMessage(Text.literal("Duel type name cannot be blank.").formatted(Formatting.RED), false);
                return;
            }
        ...
        ServerPlayNetworking.registerGlobalReceiver(NetworkConstants.DELETE_DUEL_TYPE_ID, (payload, context) -> {
            if (!SessionPermissions.checkCanManageSessions(context.player(), "delete duel types")) return;
            String id = payload.id();
            if (dev.frost.miniverse.minigame.impl.duels.DuelTypeRegistry.get(id).isEmpty()) return;
            ...
            for (dev.frost.miniverse.minigame.core.kit.Kit kit : dev.frost.miniverse.minigame.core.kit.KitRegistry.getAll()) {
                if (kit.getCategories().contains("duel_type:" + id)) {
                    ...
                    dev.frost.miniverse.minigame.core.kit.KitRegistry.saveCustomKit(updatedKit, context.server());
                }
            }
